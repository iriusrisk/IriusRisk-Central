...
def fetch_component_risk_patterns(component_id, output_path, page_size, api_token, instance_domain):
    all_items = []
    page = 0
    headers = {
    ...
    print(f"📡 Fetching risk patterns for component: {component_id}")

    while True:
        url = (
            f"https://{instance_domain}.iriusrisk.com/api/v2/components/{component_id}/risk-patterns"
    ...
    out_path = os.path.join(output_path, "component_risk_patterns.tmp")
    with open(out_path, "w") as f:
        json.dump(all_items, f, indent=2)
    print(f"\n✅ Saved {len(all_items)} risk patterns to: {out_path}")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("❌ Usage: python3 fetch_component_risk_patterns.py <component_id>")
        sys.exit(1)

    component_id = sys.argv[1]
    output_path, page_size = read_config()
    api_token, instance_domain = read_credentials()
    fetch_component_risk_patterns(component_id, output_path, page_size, api_token, instance_domain)
