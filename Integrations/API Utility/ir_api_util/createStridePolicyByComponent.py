...
class IRPolicyGenerator:
    ...
    def run(self):
        print("\n🛡️ Component-Based Threat Exclusion Drool Generator\n")
        ...
        # Fetch valid (non-deprecated) components
        components_data = self.make_api_call("/api/v2/components?filter='category.name'<>'Deprecated'&size=2000")
        components = self.extract_items(components_data)

        if not components:
            print("❌ No valid components found. Exiting.")
            sys.exit(1)

        selected_comp = self.prompt_choice("Select Component", components)
        selector_line = f'uniqueId == "{selected_comp["referenceId"]}"'
...
"""
        ...
        risk_patterns = self.extract_items(
            self.make_api_call(f"/api/v2/components/{component_id}/risk-patterns?size=2000")
        )
