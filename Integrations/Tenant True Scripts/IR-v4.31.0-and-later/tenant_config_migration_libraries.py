...
def export_libraries_to_xml(libraries_info, domain, headers):
    ...
    for library in libraries_info:
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Export failed for {library['id']}. Status: {response.status_code}. Text: {response.json()}")
    ...
    for library in libraries_info:
        if library.get('filePath') and os.path.exists(library['filePath']):
            with open(library['filePath'], 'rb') as file:
                files = {'file': (os.path.basename(library['filePath']), file, 'application/xml')}
                ...
                local_headers.pop('Content-Type', None)  # Remove content-type if set, as it will be set by requests
                response = requests.post(url, headers=local_headers, data=data, files=files)
                if response.status_code == 200:
                    logging.info(f"Imported {library['name']}.")
                else:
                    logging.error(f"Import failed for {library['name']}. Status: {response.status_code}. Response: {response.json()}")
